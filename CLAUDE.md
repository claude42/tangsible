...
**Row text (`taskLabel`/`hostLabel`/`colorTag`).** `colorTag(outcome) string` maps OK/Changed/Skipped/Failed to tag color names (`green`/`yellow`/`teal`/`red` — `teal` is `tcell`'s/W3C's closest named match for ANSI cyan). `taskLabel` right-aligns the counts segment to the far right of the row's available width (from the list's `GetInnerRect()`), truncating the *raw* title by rune count first and only escaping it afterward via `tview.Escape()` — truncating before escaping avoids ever slicing into an escape sequence `Escape()` would have produced. `tview.TaggedStringWidth()` measures visible width while ignoring the embedded color tags, which is what makes the alignment math work correctly regardless of the markup. `hostLabel` colors its whole line uniformly by its single outcome — no truncation rule applies there, that's TASK-row-specific per `TUI.md`.
